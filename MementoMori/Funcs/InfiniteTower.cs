...
public partial class MementoMoriFuncs
{
    ...
    {
        var towerType = SelectedAutoTowerType;
        await ExecuteQuickAction((log, token) =>
        {
            if (towerType == TowerType.None) return Task.CompletedTask;
            return RunTowerTypeLoop(towerType, targetStopLayer, log, token);
        });
    }
    ...
    public async Task AutoAllTowersRequest(long targetStopLayer)
    {
        await ExecuteQuickAction(async (log, token) =>
        {
            ...
            await RunTowerTypesConcurrently(towerTypes, targetStopLayer, log, token);
        });
    }

    private Task RunTowerTypesConcurrently(IEnumerable<TowerType> towerTypes, long targetStopLayer, Action<string> log, CancellationToken token)
    {
        var tasks = towerTypes.Select(t => RunTowerTypeLoop(t, targetStopLayer, log, token)).ToArray();
        return WhenAllLogExceptions(tasks, log);
    }
